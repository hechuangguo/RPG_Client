...
Common 是 **独立 Git 仓库**（[RPG_Common](https://github.com/hechuangguo/RPG_Common)），以 submodule 挂在 Client/Server 的 `Common/` 目录。**可以提交**，但不是把所有东西都塞进 Common。

| 内容 | 放哪里 | 谁提交 | 说明 |
|------|--------|--------|------|
| `.proto` 协议 | `Common/*.proto` | **RPG_Common 仓** | 客户端惯例**不直接改** proto；在 Common 子模块内改完 commit+push，再回到主仓 bump 指针（见 [`Common/README.md`](Common/README.md)） |
| 双端共用 JSON 逻辑 | `Common/map/{mapId}/` | **RPG_Common 仓** | `meta.json`、`spawns.json`、`collision.json` 等；[`MapDataLoader`](assets/_Project/Scripts/World/MapDataLoader.cs) 优先读此路径 |
...
```powershell
# 1. 在 Common 子模块内提交并推送（RPG_Common 仓）
cd Common
git add .
git commit -m "feat(map): 更新 1002 出生点"
git push origin main
cd ..

# 2. 回到 RPG_Client，只提交 submodule 指针
git add Common
git commit -m "chore: bump Common submodule"

# 或用脚本（需显式允许推 Common）
.\scripts\commit_push_all.ps1 -m "chore: bump Common" -AllowCommonEdit
```

默认 `commit_push_all.ps1` **不会**自动 push Common（防误改 proto）；改 `Common/map/*.json` 时同样需要 `-AllowCommonEdit` 或手动在 `Common/` 内 push。

**与 3D 地图方案的关系**：
- MapExport 产出的 `spawn_points.json` 等：发布到 **CDN** 的 `client/`、`server/` 目录；若需开发期双端本地联调，可同步一份到 `Common/map/` 并提交 RPG_Common
- 大块 3D 资源**绝不**进 Common；之前说的「地图资源不要放进 Common」指的是 bundle/场景，不是禁止提交 `Common/map/` 里的 JSON
