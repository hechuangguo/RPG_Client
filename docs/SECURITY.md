...
2. 客户端在 `C2SLoginReq` / `C2SRegisterReq` 中携带：
   - `login_nonce`：回显服务端 `nonce`
   - `password_digest`：`SHA-256(nonce || UTF-8(密码))`（32 字节）
   - `confirm_password_digest`（注册）：`SHA-256(nonce || UTF-8(确认密码))`
3. `ProtocolVersion`：`major=1`，`minor=1`（见 [`WireConstants.cs`](../assets/_Project/Scripts/Net/WireConstants.cs)）。

### 客户端实现

- 摘要：[`PasswordDigest.Sha256NoncePassword`](../assets/_Project/Scripts/Util/PasswordDigest.cs)
...
- [ ] 校验 `login_nonce` 与本次连接下发的 `nonce` 一致。
- [ ] 使用 `SHA-256(nonce || UTF-8(密码))` 与库中凭证比对（库侧应存慢哈希或等价安全存储，非明文）。
- [ ] 拒绝 `ProtocolVersion.minor < 1` 的旧版纯 `SHA-256(密码)` 请求（防降级）。
- [ ] 每个 TCP 连接仅接受一次 `nonce`；重连须重新下发 challenge。

### 已废弃（minor=0）

- `password_digest = SHA-256(UTF-8 密码)`，无 nonce，存在重放与彩虹表风险；**客户端不再回落**。
